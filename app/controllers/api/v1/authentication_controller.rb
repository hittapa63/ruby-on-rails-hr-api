class Api::V1::AuthenticationController < ApplicationController
    before_action :authorize_request, except: [:login, :forgot, :reset]
    ...
    def login
        ...
        if @user&.authenticate(params[:password])
            ...
            user: ActiveModelSerializers::SerializableResource.new(@user),}, status: true }, status: :ok
        else
            render json: { error: 'unauthorized', message: 'Invalid request', status: false }, status: :ok
        end
    end

    def show
        render json: { data: { user: ActiveModelSerializers::SerializableResource.new(@current_user) }, status: true }, status: :ok
    end
